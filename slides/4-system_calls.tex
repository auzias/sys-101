...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Reading a file is a blocking operation. Should writing a file be a blocking operation too? % Depends: if the program overrides the buffer right after the call the written data may be corrupted. If the kernel copies the buffer before returning into the program is does not have to block.
