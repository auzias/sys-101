...
\begin{frame}
    ...
    \begin{enumerate}
      ...
      \item Is \emph{open} required? Is there a way to avoid this and used \emph{read} or \emph{write} without calling \emph{open}?
            % open load the i-node in memory, without it you have to: 1) specify the path at each read/write, 2) load the i-node every time (even though it could be in the memory)
    \end{enumerate}
\end{frame}
